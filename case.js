...
case 'checkphone': {
    try {
        // Get target (mention, reply, or self)
        let target;
        ...
        const [userData] = await trashcore.onWhatsApp(target);
        if (!userData) return reply("❌ User not found on WhatsApp.");

        // Detect device from Baileys connection or message ID pattern
        ...
        // Detect brand/type heuristically from device info or user agent
        const ua = (trashcore.user?.platform || "WhatsApp MD").toLowerCase();
...
🏷️ Brand / Model: ${phoneBrand}
🧠 Platform: ${trashcore.user?.platform || "WhatsApp Multi-Device"}
📡 WhatsApp Version: ${trashcore.user?.waVersion?.join('.') || "Unknown"}
        `.trim();

        // Send result
        await trashcore.sendMessage(from, { text: result, mentions: [target] }, { quoted: m });
    } catch (err) {
